...
where [option] can be:"
  ...
  export_var_env DYNAWO_DEVELOPER_OPTIONS="    =========== Dynawo Developer
        ...
        jobs-valgrind ([args])                launch Dynawo simulation in valgrind (default tool to check memory leakage)
...
config_dynawo() {
  ...
  if [ -n "$DYNAWO_ZMQ_HOME" ]; then
    CMAKE_OPTIONAL="$CMAKE_OPTIONAL -DZMQ_HOME=$DYNAWO_ZMQ_HOME"
  fi
...
nrt() {
  ...
  export_var_env_force DYNAWO_LOGS_NO_TIMESTAMP=YES
...
DYNAWO_SRC_OPENMODELICA DYNAWO_ZLIB_HOME DYNAWO_LIBARCHIVE_HOME DYNAWO_BOOST_HOME DYNAWO_GTEST_HOME DYNAWO_GMOCK_HOME DYNAWO_XSD_DIR DYNAWO_PYTHON_COMMAND"
