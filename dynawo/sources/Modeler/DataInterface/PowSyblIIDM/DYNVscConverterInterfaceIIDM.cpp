...
VscConverterInterfaceIIDM::importStaticParameters() {
  ...
  if (getBusInterface()) {
    ...
    // TODO(iidm-bridge): VscConverterStation::getHvdcLine() not exposed; fall back to terminal VL nominalV.
    double vNom = vscConverterIIDM_.getTerminal().getVoltageLevel().getNominalV();
