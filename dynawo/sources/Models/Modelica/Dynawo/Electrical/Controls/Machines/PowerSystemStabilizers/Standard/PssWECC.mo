...
model PssWECC "WECC power system stabilizer (CGMES PssWECC)"
  /*
    Single-input (speed) WECC power system stabilizer: a gain, two cascaded
    washout high-pass stages, an optional torsional notch filter and two
    lead-lag phase-compensation stages. The dual washout gives a steeper
    high-pass rolloff. Corresponds to the CGMES IEC 61970-302 PssWECC
    model. Block-based: the equation section contains only connect
    statements.
  */
  ...
  parameter Types.PerUnit Kw "Stabilizer gain";
  ...
  parameter Types.Time t10 "Torsional notch filter time constant in s (0 to bypass)";
  ...
  Blocks.Math.Gain gainPss(k = Kw) "Stabilizer gain";
  Continuous.Washout washout1(tW = tW1) "First washout high-pass stage";
  Continuous.Washout washout2(tW = tW2) "Second washout high-pass stage";
  Blocks.Continuous.FirstOrder torsionalFilter(T = if t10 > 1e-6 then t10 else 1e-5) "Torsional notch filter";
...
equation
  ...
  connect(washout2.y, torsionalFilter.u);
  connect(torsionalFilter.y, leadLag1.u);
