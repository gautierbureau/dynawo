...
model GeneratorSalientPoleExp "Fifth-order salient-pole synchronous machine model with magnetic saturation"
  /*
    Fifth-order salient-pole synchronous machine with magnetic saturation: the
    GeneratorSalientPole model with a saturation factor sePu, fitted through
    two operating points, applied to the d-axis field flux state. With
    Se1Pu = 0 the saturation vanishes and the model reduces to
    GeneratorSalientPole. Corresponds to the CGMES IEC 61970-302 GenSae model
    (PSS/E GENSAE). Receptor convention; coupled to OmegaRef.
  */
...
equation
  ...
  edPPPu = - (XqPu - XppqPu) / (XqPu - XlPu) * psi2q;

  // Magnetic saturation factor (d-axis field flux only)
  sePu = satB * (sqrt(eqPPu * eqPPu) - satA) * (sqrt(eqPPu * eqPPu) - satA) / sqrt(eqPPu * eqPPu);
  ...
  if running.value then
    ...
    Tppq0 * der(psi2q) = - psi2q - (XqPu - XlPu) * Iq;
