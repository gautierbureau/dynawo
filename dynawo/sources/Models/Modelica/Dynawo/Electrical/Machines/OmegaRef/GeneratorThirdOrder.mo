...
model GeneratorThirdOrder "Third-order (one-axis, flux-decay) synchronous machine model"
  /*
    Third-order synchronous machine model: the classical electromechanical
    model (rotor angle and rotor speed) augmented with the d-axis field flux
    dynamics, so the q-axis transient EMF eqPPu becomes a state variable
    driven by the field voltage efdPu. No q-axis rotor winding is represented
    (one-axis approximation). It corresponds to the CGMES IEC 61970-302 Gen3
    model. The model uses the receptor convention (i > 0 when entering the
    device) and is coupled to the OmegaRef frequency handling.
  */
...
equation
  ...
  // (theta is the angle of the q-axis; generator current = - terminal.i, machine base via kk)
