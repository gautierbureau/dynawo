...
model GeneratorRoundRotorTypeJ "Sixth-order round-rotor synchronous machine model with WECC air-gap-flux and stator-current (GENTPJ) saturation"
  /*
    Sixth-order round-rotor synchronous machine using the WECC GENTPJ
    saturation treatment: same as GeneratorRoundRotorTypeF (GENTPF) but with
    the saturation argument augmented by a stator-current term Kis*|I|, which
    captures the increase in magnetic reluctance caused by leakage flux
    induced in the stator teeth by high stator currents. With Kis = 0 the
    model reduces to GeneratorRoundRotorTypeF.
    Corresponds to CGMES IEC 61970-302 SynchronousMachineTimeConstantReactance
    with rotorKind = roundRotor and modelKind = subtransientTypeJ (PSS/E GENTPJ).
  ...
  */
  ...
  parameter Types.PerUnit KisPu "Stator-current saturation coefficient in pu (multiplier on |I|, base SNom)";
  ...
  Types.PerUnit psiArgPu(start = PsiArg0Pu) "Saturation function argument (air-gap flux + Kis*|I|) in pu (base UNom)";
  Types.PerUnit satDPu(start = SatD0Pu) "d-axis saturation factor SatD = 1 + S(psiArg)";
  Types.PerUnit satQPu(start = SatQ0Pu) "q-axis saturation factor SatQ = 1 + (Xq/Xd)*S(psiArg)";
  ...
  final parameter Types.PerUnit PsiArg0Pu = PsiAg0Pu + KisPu * sqrt(Id0 * Id0 + Iq0 * Iq0) "Start value of saturation function argument";
  final parameter Types.PerUnit Sat0Pu = if PsiArg0Pu > 0 then satB * (PsiArg0Pu - satA) * (PsiArg0Pu - satA) / PsiArg0Pu else 0 "Start value of saturation function S(psiArg)";
...
equation
  ...
  // Air-gap flux magnitude, plus the GENTPJ stator-current term in the saturation argument
  psiAgPu = sqrt((Vq + Iq * RaPu + Id * XlPu) ^ 2 + (Vd + Id * RaPu - Iq * XlPu) ^ 2);
  psiArgPu = psiAgPu + KisPu * sqrt(Id * Id + Iq * Iq);

  // Multiplicative saturation factors (scaled-quadratic), per PowerWorld GENTPJ
  // (max() guards the 1/psiArg singularity when the machine is fully de-energised)
  satDPu = 1 + satB * (psiArgPu - satA) * (psiArgPu - satA) / max(psiArgPu, 1e-6);
  satQPu = 1 + XqPu / XdPu * satB * (psiArgPu - satA) * (psiArgPu - satA) / max(psiArgPu, 1e-6);
