...
model GeneratorSalientPoleTypeJ "Fifth-order salient-pole synchronous machine model with WECC air-gap-flux and stator-current (GENTPJ) saturation"
  /*
    Fifth-order salient-pole synchronous machine using the WECC GENTPJ
    saturation treatment: same as GeneratorSalientPoleTypeF (GENTPF) but with
    the saturation argument augmented by a stator-current term Kis*|I|. With
    Kis = 0 the model reduces to GeneratorSalientPoleTypeF.
    Corresponds to CGMES IEC 61970-302 SynchronousMachineTimeConstantReactance
    with rotorKind = salientPole and modelKind = subtransientTypeJ (PSS/E GENTPJ).
...
equation
  ...
  if running.value then
    ...
    // Subtransient q-axis flux dynamics (no Ed' state for salient pole)
    Tppq0 * der(edPPPu) = -satQPu * edPPPu + (XqPu - XlPu) * Iq;
