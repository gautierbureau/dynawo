...
model GeneratorSalientPoleTypeF "Fifth-order salient-pole synchronous machine model with WECC air-gap-flux (GENTPF) saturation"
  /*
    Fifth-order salient-pole synchronous machine using the WECC GENTPF
    saturation treatment: same multiplicative SatD/SatQ scheme as
    GeneratorRoundRotorTypeF, with no q-axis transient state (Ed' = 0). The
    q-axis subtransient state Ed'' is therefore driven directly by the stator
    current through (Xq - Xl)*Iq, mirroring the GENSAL/GENSAE structure. With
    Se1Pu = 0 the saturation vanishes and the model reduces to
    GeneratorSalientPole (modulo the GENTPF state-space parameterisation).
    Corresponds to CGMES IEC 61970-302 SynchronousMachineTimeConstantReactance
    with rotorKind = salientPole and modelKind = subtransientTypeF (PSS/E GENTPF).
